...
task_result * BM1370_process_work(void * pvParameters)
{
    ...
    // Handle job response
    uint8_t job_id = (asic_result.job.job_id & 0xf0) >> 1;
    uint8_t core_id = (uint8_t)((ntohl(asic_result.job.nonce) >> 25) & 0x7f); // BM1370 has 80 cores, so it should be coded on 7 bits
    uint8_t small_core_id = asic_result.job.job_id & 0x0f; // BM1370 has 16 small cores, so it should be coded on 4 bits

...
{
    return i2c_bitforge_register_read(tps546_i2c_handle, command, data, 1);
}
...
{
    return i2c_bitforge_register_write_byte(tps546_i2c_handle, command, data);
}
...
{
    return i2c_bitforge_register_write_addr(tps546_i2c_handle, command);
}
...
{
    uint8_t data[2];
    if (i2c_bitforge_register_read(tps546_i2c_handle, command, data, 2) != ESP_OK) {
        return ESP_FAIL;
    } else {
...
{
    return i2c_bitforge_register_write_word(tps546_i2c_handle, command, data);
}
...
static esp_err_t smb_read_block(uint8_t command, uint8_t *data, uint8_t len)
{
    ...
    if (i2c_bitforge_register_read(tps546_i2c_handle, command, buf, len+1) != ESP_OK) {
        free(buf);
        return ESP_FAIL;
    }
...
//     //write it all
//     if (i2c_bitforge_register_write_bytes(tps546_i2c_handle, buf, len+2) != ESP_OK) {
//         free(buf);
//         return ESP_FAIL;
//     } else {
//         free(buf);
...
esp_err_t TPS546_init(TPS546_CONFIG config)
{
    ...
    ESP_RETURN_ON_ERROR(i2c_bitforge_add_device(TPS546_I2CADDR, &tps546_i2c_handle, TAG), TAG, "Failed to add TPS546 I2C");

...
esp_err_t DS4432U_init(void) {
    return i2c_bitforge_add_device(DS4432U_SENSOR_ADDR, &ds4432u_dev_handle, TAG);
}
...
esp_err_t DS4432U_set_current_code(uint8_t output, uint8_t code) {
    ...
    return i2c_bitforge_register_write_byte(ds4432u_dev_handle, reg, code);
}
...
esp_err_t DS4432U_get_current_code(uint8_t output, uint8_t *code) {
    uint8_t reg = (output == 0) ? DS4432U_OUT0_REG : DS4432U_OUT1_REG;
    return i2c_bitforge_register_read(ds4432u_dev_handle, reg, code, 1);
}
...
esp_err_t DS4432U_test(void)
{
    ...
    ESP_RETURN_ON_ERROR(i2c_bitforge_register_read(ds4432u_dev_handle, DS4432U_OUT0_REG, &data, 1), TAG, "Failed to read DS4432U+ OUT0 register");
